...
var pack = d3.layout.tree().size([100, 100])
...
var linkSvg = vis.append("g").attr("class", "linkContainer");

scope.deleteLink = function(name, pname, i){

    var d = scope.current_node;
  //console.log(scope.current_node);
  //console.log(scope.json.name + ' ' + name);
    //if (name == scope.json.name && pname == "") {
    //    console.log('here');
    //    scope.json.link.splice(i,1);
    //    return;
    //}
    d.link.splice(i, 1);

    var visited = new Array();
    // var stack = new Array($scope.json)
...
function showToolTip(d) {
    ...
    for (var i in d.link) {
        ...
        var url = d.link[i]['url'];

        var a = angular.element('<a href="' + url + '" target="_blank">'
                +name +' </a>');

        angular.element(document.getElementById('myToolTip')).append(a);

        var parentname = "";
        if (d.parent) parentname = d.parent.name;

        var text = '<button ng-click=\'deleteLink("'+d.name+'","' +
                parentname+'",'+i+')\'>-</button></br>';

        angular.element(document.getElementById('myToolTip')).append($compile(text)(scope));
    }
